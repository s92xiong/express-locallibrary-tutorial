...
function authorCreate(first_name, family_name, d_birth, d_death, cb) {
  ...
  author.save(function (err) {
    // The API uses the error-first argument, convention, so the 1st argument for the cb will always be an error value (or null)
    // If the API returns some result, this will be provided as the 2nd argument
    if (err) {
      cb(err, null)
      return
    }
